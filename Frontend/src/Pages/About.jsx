...
function About(){
    ...
    return(
     <div className="box-border scroll-smooth overflow-x-hidden">
       ...
       <div className="flex justify-center relative mt-30 md:justify-normal md:mx-10">
          <p className="text-3xl text-transparent font-extrabold bg-clip-text bg-linear-to-r from-white to-gray-200">About</p>
       </div>
       <div className="relative flex mt-5 md:mx-10 ">
        <div className="flex flex-col m-5  md:flex-row md:m-0 items-center p-5 md:gap-10  md:w-[80%] rounded-md backdrop-blur bg-stone-900/40 border border-gray-100/30">
          <img src={assets.profile_icon} className="w-40 rounded-full object-cover" />
          <About_Me />
        </div>
      </div> 
      <div className="grid grid-cols-2 relative mt-6 w-80" >
        <div className=" flex flex-row items-center justify-center text-green-300 mx-7 md:mx-15 rounded-md w-35 h-15 gap-2 border border-gray-100/40" >
          <SiWhatsapp className="text-lg font-semibold"/>
          <span className="text-md font-semibold" >0792781047</span>
        </div>
         <div className="flex items-center justify-center  text-[#f7f0ff] mx-15 rounded-md w-33 h-15 border border-gray-100/40 gap-2 transition delay-30 duration-120 ease-in-out hover:-translate-y-1 hover:scale-110 hover:text-indigo-500">
       ...
       <div className="flex flex-row justify-center  relative mt-10 mx-10 gap-2 md:justify-normal">
        <FaGraduationCap className="text-2xl text-green-300" />
         <span className="font-semibold text-2xl mb-5 text-[#f0f7ff]"> Education</span>
       </div>
       <div className=" md:h-80 md:w-[85%] mx-5 md:mx-10 text-[#f0f7ff] divide-y divide-white/40 md:p-10 rounded-md backdrop-blur bg-stone-900/40 border border-gray-100/30">
             {
            education.map(skill =>(             
              <div className=" flex flex-col md:flex-row md:justify-between p-5 text-[f0f7ff]">
                <div className=" flex flex-col space-y-2">
                  <span className="text-xl font-bold ">{skill.sch}</span>
                  <span className=" text-lg md:text-md font-semibold text-green-300">{skill.degree}</span>
                  <span className=" text-lg md:text-md text-gray-300">{skill.location}</span>
                </div>
                <div className="flex flex-col space-y-2 ">
      ...
      <div className="flex flex-row justify-center md:justify-normal relative mt-10 mx-10 gap-2">
          <IoMdBriefcase className="text-2xl text-green-300"/>
          <span className="font-semibold text-2xl mb-5 text-[#f7f0ff]">Experience</span>
      </div>  
    <div className=" md:w-[85%] mx-5 md:mx-10 text-[#f0f7ff] divide-y divide-white/40 md:p-10 rounded-md backdrop-blur bg-stone-900/40 border border-gray-100/30">
              ...
              <div className="flex flex-col md:flex-row md:justify-between gap-4 p-5">
                ...
                {example.jobs.map((job, jobIdx) => (
                  <li className="p-1">
                    ...
                    <span key={jobIdx} className="text-md">{job}</span>
                  </li>
                ))}
                </ul>
              </div>
               <div className="flex flex-col text-[#f0f7ff] gap-2">
                 <p className="text-md font-bold text-gray-300">{example.start} - {example.end}</p>
     ...
     <div className="flex flex-row justify-center md:justify-normal relative mt-10 mx-10 gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 mx-10 md:w-[85%] relative gap-5">
     ...
     <div className="flex flex-row justify-center md:justify-normal relative mt-10 mx-10 gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 mx-10 md:w-[90%] p-5 divide-y md:divide-y-0 md:divide-x divide-white/40 relative bg-stone-900/40 border border-gray-100/30 rounded-md"> 
       <div className="mx-5">
        {
          frontend.map(front =>(
      ...
      <div className="flex flex-row relative justify-center items-center mt-10 mb-5 gap-2 p-2">
        <FaProjectDiagram className="text-green-300 text-2xl md:text-3xl"/>
      ...
      <div className=" backdrop-blur bg-stone-900/40 border border-gray-100/30 rounded-lg p-5 text-[#f7f0ff] mt-20 mx-5">
